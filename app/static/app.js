...
const METRIC_HELP = {
  ...
  delta: ["Δ к предыдущему периоду",
    "Изменение конверсии по сравнению с предыдущим отрезком той же длины. Зелẽный — рост, красный — падение. Пусто, если за предыдущий период данных в базе нет.",
  ...
  top: ["Топы номенклатуры",
    "Позиции ранжируются по конверсии КП→ЗК. В расчёт попадают только те, что встретились минимум в N предложениях — иначе в топ выходят позиции с одним КП и одним заказом, то есть конверсией 100%. Порог задаẽтся селектором «Мин. КП».",
...
function renderKpis(d) {
  $("kpis").innerHTML = KPI_DEFS.map((k) => `
    <div class="kpi card">
      ...
      <canvas class="spark" id="sp_${k.key}"></canvas>
    </div>`).join("");
